...
map = [
    [i // 2 + 1 if (i + j) % 2 == 0 and i % 2 == 1 else i // 2 if (i + j) % 2 == 1 and i % 2 == 1 else i // 2 - 1 if (i + j) % 2 == 1 and i % 2 == 0 else i // 2 + 1 for j in range(cols)]
...
while running:
    ...
    # If Game is running
    if play:
        # Add the current ball position to the memory trail
        memory_trail.append((ball_pos[0], ball_pos[1]))
        # Keep only the last few positions to create the memory trail effect
        memory_trail = memory_trail[-150//ball_speed:]
        ...
        if (paddle < ball_pos[0] < paddle + paddle_width and HEIGHT - BALL_SIZE - paddle_height <= ball_pos[1] <=
                HEIGHT - BALL_SIZE + ball_speed):
            ball_dir[1] = -ball_dir[1]
    ...
    # Draw the memory trail
    for i in range(min(len(memory_trail), 150 // ball_speed)):
        mem_pos = memory_trail[i]
        trail_color = (int(3+(1+ ball_speed/30) ** i + ball_speed // 1.2 * i), ball_speed * i, ball_speed * i)  # Use ** for exponentiation
        pygame.draw.ellipse(screen, trail_color, (mem_pos[0], mem_pos[1], BALL_SIZE, BALL_SIZE))
